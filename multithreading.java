...
System.out.println ("Match:"+p); p=primer.read(); 
f=fibr.read(); 
 
} 
 
else if(f<p) f=fibr.read(); else p=primer.read(); 
}catch(IOException e) 
...
int s=(int)Math.sqrt(n); for(int i=2;i<=s;i++) 
{ 
...
{public static void main (String[] args)throws IOException { fibonacci fib=new fibonacci(); prime prime=new prime(); receiver r=new receiver(fib,prime); 
fib.start(); 
prime.start(); 
r.start(); 
